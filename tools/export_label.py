...
class DemoDataset(DatasetTemplate):
    ...
    def __getitem__(self, index):
        ...
        if points.shape[1] == 4:
            timestamp_col = np.zeros((points.shape[0], 1), dtype=np.float32)
            points = np.concatenate([points, timestamp_col], axis=1)
...
if __name__ == '__main__':
    main()
#python tools/export_label.py --cfg_file tools/cfgs/nuscenes_models/cbgs_voxel0075_voxelnext_doubleflip.yaml --data_path Test/ --ckpt voxelnext_nuscenes_kernel1.pth --ext .bin --output_dir output_results
